		     ...
		     int erase_flag, int no_more_flag)
{
    ...
    if (height < 1)
    {
        printf("called with non-positive window height %d, forcing to 1", height);
        height = 1;
    }
    else if (height >= 2)
        height--;
    ...
    if (width < 1)
    {
        printf("called with non-positive window width %d, forcing to 1", width);
        width = 1;
    }
    ...
    for (data = b->head; data && (height > 0); data = data->next)
    {
        ...
        /* This should not ordinarily happen. */
        {
            ...
            if (iov != small_iov)
            {
                printf("growing iov array to %d; width %d, height %d, size %lu", 
                        iov_alloc, width, height, (u_long)b->size);
                iov = XREALLOC(MTYPE_TMP, iov, iov_alloc*sizeof(*iov));
            }
            else
            {
                /* This should absolutely never occur. */
                printf("corruption detected: iov_small overflowed; "
                        "head %p, tail %p, head->next %p",
                        b->head, b->tail, b->head->next);
                iov = XMALLOC(MTYPE_TMP, iov_alloc*sizeof(*iov));
    ...
    example: Solaris2.6 are defined IOV_MAX size at 16.     */
    {
        ...
        while (iov_index > 0)
        {
            ...
            {
                //printf("writev to fd %d failed: %s", fd, safe_strerror(errno));
                break;
            }
...
#define MAX_FLUSH 131072
    ...
    while (written > 0)
    {
        ...
        if (!(d = b->head))
        {
            printf("corruption detected: buffer queue empty,but written is %lu",
                    (u_long)written);
            break;
        }
